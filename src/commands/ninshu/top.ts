 ...
 * similiar to other discord message function, this one sends top 10 players as image records
...
async function send_image_records_to_discord(message: Message, array: string | any[]) {
  ...
  records.push(
    " " + "    Player Name".padStart(12, " ") + " " + "  Kill Count ".padStart(8, " ") + " \n"
  );
...
async function jimp(records: string[]) {
  const fontLeaderBoard = await Jimp.loadFont(
    "./src/font/font_leaderboard/5r0rI6Ck6aU2LYVyLdMfsfDs.ttf.fnt"
  );
